...
interface FourHundredGigE0/0/0/1
 description Connected_to_P3_1
 mtu 9216
 ipv4 address 198.51.101.1 255.255.255.0
 ipv4 unreachables disable
 load-interval 30
 damp
 !
...
!
interface FourHundredGigE0/0/0/1
 description Connected_to_P1_1
 mtu 9216
 ipv4 address 198.51.101.2 255.255.255.0
 ipv4 unreachables disable
 load-interval 30
 dampening
 !
...
def get_ssh_cmd(sim, device, is_server=False):
    ...
    console_ports = sim.ports()
    if (is_server):
        return "ssh root@" + str(console_ports[device]['HostAgent']) + ' -p' + str(console_ports[device]['xr_redir23'])
    else:
        return "ssh cisco@" + str(console_ports[device]['HostAgent']) + ' -p' + str(console_ports[device]['xr_redir23'])
